// modules/dashboard/cloudinary/upload.news

import type { CloudinaryImage } from "@/shared/types/cloudinary.type";

export const newsUploadHelper = {
  uploadPhoto: async (file: File, name?: string): Promise<CloudinaryImage> => {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("folder", "village/news");
    formData.append("publicId", `village/news/${name || "news"}-${Date.now()}`);
